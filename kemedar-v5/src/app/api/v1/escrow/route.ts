...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await escrowRepository.findMany(
      { buyerId: params.get("buyerId") || undefined, sellerId: params.get("sellerId") || undefined, status: params.get("status") || undefined },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const deal = await escrowService.createDeal(body);
