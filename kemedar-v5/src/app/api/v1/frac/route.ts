...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await fracRepository.findMany(
      { status: params.get("status") || undefined, isFeatured: params.get("isFeatured") === "true" ? true : undefined },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const offering = await fracService.submitOffering(session.userId, body);
    return successResponse(offering, 201);
  } catch (error: any) {
