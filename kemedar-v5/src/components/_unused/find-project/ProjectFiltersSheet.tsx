...
export default function ProjectFiltersSheet({ open, activeKey, onClose, filters, setFilters }) {
  ...
  useEffect(() => {
    ...
    if (citySearch.trim().length < 1) {
      // Load defaults
      apiClient.get("/api/v1/" + "city", "-created_date", 50)
    ...
    Promise.all([
      apiClient.get("/api/v1/" + "city", undefined, 200),
      apiClient.get("/api/v1/" + "district", undefined, 200),
      apiClient.get("/api/v1/" + "area", undefined, 200),
