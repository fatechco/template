...
export default function FinishingSimulatorResults({ simulation, property, onClose }) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      setLoading(true);
      const res = await apiClient.post("/api/v1/ai/generateFinishingEstimatePDF", {
        simulationId: simulation.id
      });
      // Trigger download
  ...
  const handleKemeworkRedirect = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiClient.post("/api/v1/ai/redirectToKemeworkTask", {
        simulationId: simulation.id
      });
  ...
  const handleKemetroRedirect = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiClient.post("/api/v1/ai/redirectToKemetroRFQ", {
        simulationId: simulation.id
      });
  ...
  const handleTurnkeyRequest = async () => {
    ...
    setLoading(true);
    try {
      await apiClient.post("/api/v1/ai/notifyTurnkeyLead", {
        simulationId: simulation.id
      });
