...
export default function ServiceOrderDetailModal({ order, service, module: mod, onClose, onRefresh, onAssign }) {
  ...
  useEffect(() => {
    apiClient.list("/api/v1/serviceorderactivity", { serviceOrderId: order.id }, "-created_date")
      .then(setActivities).catch(() => setActivities([]));
  }, [order.id]);

  const saveNotes = async () => {
    setSavingNotes(true);
    await apiClient.put("/api/v1/serviceorder/", order.id, { adminNotes, franchiseNotes });
