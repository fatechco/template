...
export default function AdminAuctionWidget() {
  ...
  useEffect(() => {
    Promise.all([
      apiClient.list("/api/v1/propertyauction", { status: 'live' }).catch(() => []),
      apiClient.get("/api/v1/" + "auctionbid", '-bidPlacedAt', 200).catch(() => []),
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/admin/kemedar/auctions" className="text-xs text-orange-500 font-bold hover:underline">
          View Full Dashboard →
        </Link>
      </div>
