...
export default function CoachChat({ profile, currentStep }) {
  ...
  const loadMessages = async () => {
    const msgs = await apiClient.list("/api/v1/coachmessage", { coachProfileId: profile.id }, '-created_date', 20);
  ...
  const sendMessage = async (text) => {
    ...
    setInput('');
    setLoading(true);
    const res = await apiClient.post('/api/v1/ai/generateCoachResponse', {
              ...
              {msg.attachedContent?.featureName && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
                  ...
                  {msg.attachedContent.featureUrl && (
                    <Link href={msg.attachedContent.featureUrl}
