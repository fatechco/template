...
export default function CoachDashboardWidget() {
  ...
  useEffect(() => {
    apiClient.get("/api/auth/session")
      .then(u => u ? apiClient.list("/api/v1/coachprofile", { userId: u.id }) : [])
  ...
  if (!profile) {
    return (
      <div className="bg-gradient-to-br from-teal-500 to-orange-500 rounded-2xl p-5 text-white">
        ...
        <Link href={coachPath} className="block w-full text-center bg-white text-orange-600 font-black py-2.5 rounded-xl hover:bg-orange-50 transition-colors text-sm">
          🚀 Start My Journey
        </Link>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-2xl border-l-4 border-l-teal-500 border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex gap-2">
        <Link href={coachPath} className="flex-1 text-center bg-orange-500 hover:bg-orange-600 text-white font-black py-2 rounded-xl text-xs transition-colors">
          ▶️ Continue
        </Link>
        <Link href={coachPath + '?tab=chat'} className="flex-1 text-center border border-gray-200 text-gray-600 font-bold py-2 rounded-xl text-xs hover:border-teal-300 transition-colors">
          💬 Ask Coach
        </Link>
      </div>
    </div>
