...
import { Outlet, useLocation, Link } from 'next/navigation';
import { Menu, X, LogOut, HelpCircle, ChevronDown, ChevronRight } from 'lucide-react';
import { useAuth } from '@/lib/auth-context';

function NavItem({ item, isActive }) {
  const pathname = usePathname();
  const hasActiveChild = item.children?.some(c => !c.heading && c.to && pathname.startsWith(c.to));
        ...
        {open && (
          <div className="ml-4 pl-3 border-l border-gray-200 mt-0.5 space-y-0.5">
            {item.children.map((child, ci) => {
              ...
              const active = child.to && pathname === child.to;
              return (
                <Link
                  key={child.to}
                  href={child.to || "#"}
        ...
        {sidebarOpen && (
          <div className="p-4 border-b border-gray-100">
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="/dashboard/profile" className="text-xs text-gray-400 hover:text-gray-700">Edit Profile →</Link>
            </div>
          </div>
        )}
