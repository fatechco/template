...
import { Outlet, useLocation, Link } from "next/link";
import {
...
function NavItem({ item, isActive }) {
  ...
  const hasActiveChild = item.children?.some(
    c => !c.heading && c.to && (pathname === c.to || pathname.startsWith(c.to))
  );
      ...
      <aside className={`${sidebarOpen ? "w-[260px]" : "w-0 overflow-hidden"} flex-shrink-0 bg-white border-r border-gray-200 flex flex-col transition-all duration-300`}>
        ...
        <div className="px-5 py-5 border-b border-gray-100">
          ...
          <div className="flex items-center justify-between">
            ...
            <Link href={profileTo} className="text-xs text-gray-400 hover:text-gray-700 transition-colors">Edit Profile →</Link>
          </div>
        </div>
