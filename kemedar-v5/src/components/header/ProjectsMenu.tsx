      ...
      <div className="flex">
        ...
        <div className="flex-1 py-4 px-5 border-r border-gray-100">
          ...
          {FEATURED_CITIES.map((city) => (
            <Link key={city} href={`/kemedar/search-projects?city=${encodeURIComponent(city)}`} className="flex items-center gap-2 w-full text-left px-2 py-2 text-sm text-gray-700 hover:text-[#FF6B00] hover:bg-orange-50 rounded transition-colors">
        ...
        <div className="flex-1 py-4 px-5">
          ...
          <div className="columns-2 gap-2">
            {PROVINCES.map((prov) => (
              <Link key={prov} href={`/kemedar/search-projects?province=${encodeURIComponent(prov)}`} className="block w-full text-left px-2 py-1.5 text-xs text-gray-600 hover:text-[#FF6B00] hover:bg-orange-50 rounded transition-colors">
                {prov}
              </Link>
            ))}
          </div>
