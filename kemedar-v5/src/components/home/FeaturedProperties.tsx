...
function PropertyCard({ property, index }) {
  ...
  return (
    <Link href={`/property/${property.id}`} className="flex-shrink-0 w-64 bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group">
      {/* Image */}
...
export default function FeaturedProperties() {
  ...
  useEffect(() => {
    setLoading(true);
    apiClient.list("/api/v1/property", { is_featured: true, is_active: true }, "-created_date", 20)
      .then(setProperties)
      .catch(() => setProperties([]))
