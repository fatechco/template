...
export default function NEARWalletSection({ wallet, setWallet, user }) {
  ...
  const handleConnect = async () => {
    ...
    try {
      const updated = wallet?.id
        ? await apiClient.put("/api/v1/blockchainwallet/", wallet.id, { walletAddress: walletInput.trim() })
        : await apiClient.post("/api/v1/blockchainwallet", { userId: user?.id, walletAddress: walletInput.trim() });
      setWallet(updated);
      setShowInput(false);
    } catch {
      // mock success
  ...
  const handleDisconnect = async () => {
    if (!wallet?.id) return;
    await apiClient.put("/api/v1/blockchainwallet/", wallet.id, { walletAddress: null }).catch(() => {});
