...
export default function SurplusMarketWidget() {
  ...
  useEffect(() => {
    Promise.all([
      apiClient.get("/api/v1/" + "surplusitem", "-created_date", 100).catch(() => []),
      apiClient.get("/api/v1/" + "surplustransaction", "-created_date", 50).catch(() => []),
    ]).then(([it, tr]) => {
      setItems(it || []);
      setTransactions(tr || []);
    });
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/admin/kemetro/surplus" className="text-xs text-green-600 font-bold hover:underline">View Full Dashboard →</Link>
      </div>
      ...
      <Link href="/admin/kemetro/surplus" className="block w-full text-center border-2 border-green-500 text-green-700 font-black py-2.5 rounded-xl text-sm hover:bg-green-50 transition-all">
