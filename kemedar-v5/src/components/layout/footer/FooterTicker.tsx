    ...
    try {
      ...
      const [soldItems, ecoTxList, kitsTemplates, liveAuctions, extAuctions, fracProps, escrowAuctions, snapToday] =
        await Promise.allSettled([
          apiClient.list("/api/v1/surplusitem", { status: "sold" }, "-created_date", 500),
          apiClient.list("/api/v1/surplustransaction", { transactionType: "settlement" }, "-created_date", 500),
          apiClient.get("/api/v1/" + "kemekittemplate", "-created_date", 200),
          apiClient.list("/api/v1/propertyauction", { status: "live" }, "-created_date", 50),
          apiClient.list("/api/v1/propertyauction", { status: "extended" }, "-created_date", 50),
          apiClient.get("/api/v1/" + "fracproperty", "-created_date", 200),
          apiClient.list("/api/v1/propertyauction", { status: "legal_transfer" }, "-created_date", 200),
          apiClient.list("/api/v1/snapsession", { created_date: { $gte: today } }, "-created_date", 500),
        ]);
  ...
  const loadPlatform = async () => {
    try {
      const [props, projects, products, stores, tasks, services, users, countries] =
        await Promise.allSettled([
          apiClient.list("/api/v1/property", { is_active: true }, "-created_date", 1),
          apiClient.list("/api/v1/project", {}, "-created_date", 1),
          apiClient.list("/api/v1/kemetroproduct", { isActive: true }, "-created_date", 1),
          apiClient.get("/api/v1/" + "kemetrostore", "-created_date", 1),
          apiClient.get("/api/v1/" + "kemeworktask", "-created_date", 1),
          apiClient.list("/api/v1/kemeworkservice", { isActive: true }, "-created_date", 1),
          apiClient.get("/api/v1/" + "user", "-created_date", 1),
          apiClient.get("/api/v1/" + "country", "-created_date", 1),
        ]);
      ...
      const [p2, pr2, pd2, st2, tk2, sv2, us2] = await Promise.allSettled([
        apiClient.list("/api/v1/property", { is_active: true }, "-created_date", 1000),
        apiClient.get("/api/v1/" + "project", "-created_date", 500),
        apiClient.list("/api/v1/kemetroproduct", { isActive: true }, "-created_date", 1000),
        apiClient.get("/api/v1/" + "kemetrostore", "-created_date", 500),
        apiClient.get("/api/v1/" + "kemeworktask", "-created_date", 1000),
        apiClient.list("/api/v1/kemeworkservice", { isActive: true }, "-created_date", 1000),
        apiClient.get("/api/v1/" + "user", "-created_date", 1000),
      ]);
      const g = (r) => (r.status === "fulfilled" ? r.value || [] : []);
