  ...
  useEffect(() => {
    const fetchScore = async () => {
      try {
        ...
        const data = await apiClient.list("/api/v1/neighborhoodlifescore", query, undefined, 1);
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="p-5 flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <Link href={detailUrl} className="text-sm text-orange-500 hover:underline flex items-center gap-1">
            📍 {score.displayName}
          </Link>
        </div>
        ...
        <div className="px-5 pb-5 border-t border-gray-100">
          ...
          {score.aiSummary && (
            <p className="text-sm text-gray-600 leading-relaxed mt-3 mb-3">
              ...
              <Link href={detailUrl} className="text-orange-500 hover:underline ml-1">
          ...
          <Link href={detailUrl} className="block w-full text-center text-sm font-bold text-orange-500 border-2 border-orange-500 py-2 rounded-xl hover:bg-orange-50 transition-colors mt-2">
