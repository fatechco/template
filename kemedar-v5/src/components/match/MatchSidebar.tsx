...
export default function MatchSidebar({ userId, matchCount = 0 }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiClient.list("/api/v1/propertyswipe", { userId }, '-created_date', 20),
      apiClient.list("/api/v1/propertymatch", { buyerId: userId }, '-created_date', 10)
    ]).then(([sw, ma]) => {
      setSwipes(sw);
      setMatches(ma);
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 overflow-hidden">
              ...
              matches.length === 0 ? (
                ...
                return (
                  <div key={match.id} className="bg-white/10 border border-orange-500/30 rounded-xl p-3">
                    ...
                    <button
                      onClick={() => router.push(`/kemedar/negotiate/${match.id}`)}
      ...
      <div className="p-3 border-t border-white/10 flex-shrink-0">
        <button onClick={() => router.push("/kemedar/match/history")}
