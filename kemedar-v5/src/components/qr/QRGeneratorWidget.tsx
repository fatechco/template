...
export default function QRGeneratorWidget({ targetType, targetId, targetTitle, mode = 'compact' }) {
  ...
  useEffect(() => {
    apiClient.get("/api/auth/session").then(user => {
      if (!user) { setHasPaidPlan(false); return; }
      // Check active subscription
      apiClient.list("/api/v1/usersubscription", { status: 'active' })
  ...
  useEffect(() => {
    ...
    apiClient.post('/api/v1/ai/getUserQRCodes', { targetType })
      .then(res => {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      if (qrCode?.id) {
        // Regenerate existing
        const res = await apiClient.post('/api/v1/ai/regenerateQRCode', {
          qrCodeId: qrCode.id,
          ...settings,
        });
        ...
        // Generate new
        const res = await apiClient.post('/api/v1/ai/generateQRCode', {
          targetType,
          targetId,
          ...settings,
        });
