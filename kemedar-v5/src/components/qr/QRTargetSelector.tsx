...
export default function QRTargetSelector({ onClose }) {
  ...
  const loadItems = async (type) => {
    ...
    try {
      if (type === 'property') {
        const res = await apiClient.list("/api/v1/property", {}, '-created_date', 50);
        setItems(res.map(p => ({ id: p.id, label: p.title })));
      } else if (type === 'project') {
        const res = await apiClient.list("/api/v1/project", {}, '-created_date', 50);
        setItems(res.map(p => ({ id: p.id, label: p.title || p.name })));
      } else if (type === 'product') {
        const res = await apiClient.list("/api/v1/kemetroproduct", {}, '-created_date', 50);
        setItems(res.map(p => ({ id: p.id, label: p.name })));
      } else if (type === 'store') {
        const res = await apiClient.list("/api/v1/kemetrostore", {}, '-created_date', 10);
        setItems(res.map(s => ({ id: s.id, label: s.storeName || s.name })));
      } else if (type === 'service') {
        const res = await apiClient.list("/api/v1/kemeworkservice", {}, '-created_date', 50);
