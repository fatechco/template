        ...
        <div className="space-y-3">
          {QUICK_WINS.map((win, i) => (
            <div key={i} className="bg-white rounded-2xl border border-l-4 border-l-teal-400 border-gray-100 shadow-sm p-4">
              ...
              <Link href={win.to} className="mt-3 inline-block text-xs bg-orange-500 text-white font-bold px-4 py-2 rounded-xl hover:bg-orange-400 transition-colors">
                {win.action}
              </Link>
            </div>
          ))}
          ...
          {MEDIUM_TERM.map((item, i) => (
            <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-4">
              ...
              <div className="text-right flex-shrink-0">
                ...
                <Link href={item.to || "#"} className="text-xs text-orange-500 font-bold hover:underline">{item.action}</Link>
              </div>
            </div>
          ))}
        </div>
