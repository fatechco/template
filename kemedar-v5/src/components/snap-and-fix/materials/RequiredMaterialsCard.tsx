...
export default function RequiredMaterialsCard({ session, onSessionUpdate }) {
  ...
  const handleDecisionChange = async (val) => {
    ...
    try {
      await apiClient.put("/api/v1/snapsession/", session.id, { materialsSuppliedBy: val });
    } catch { /* silent */ }
  ...
  const handleAddAllToCart = async () => {
    ...
    for (let i = 0; i < materials.length; i++) {
      ...
      try {
        ...
        await apiClient.post("/api/v1/snapmaterialcartitem", {
    ...
    await apiClient.put("/api/v1/snapsession/", session.id, {
