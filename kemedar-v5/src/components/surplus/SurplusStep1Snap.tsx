...
export default function SurplusStep1Snap({ capturedImages, setCapturedImages, onAiResult, onManual }) {
  ...
  const handleImageCapture = async (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      const res = await apiClient.post("/api/v1/ai/generateSurplusListing", {
        imageBase64: base64,
        mediaType,
      });
      setLoading(false);
  ...
  const handleBarcodeSubmit = async () => {
    ...
    setLoading(true);
    setMsgIndex(0);
    const res = await apiClient.post("/api/v1/ai/generateSurplusListing", { barcodeString: barcode.trim() });
    setLoading(false);
    onAiResult(res.data);
  };
      ...
      <div className="p-4 space-y-4 max-w-lg mx-auto">
        ...
        <button
          onClick={async () => {
            ...
            setLoading(true);
            setMsgIndex(0);
            const res = await apiClient.post("/api/v1/ai/generateSurplusListing", { imageBase64: base64, mediaType });
            setLoading(false);
            onAiResult(res.data);
          }}
