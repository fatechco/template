...
export default function VerifyProPanel({ property, user }) {
  ...
  useEffect(() => {
    ...
    apiClient.list("/api/v1/propertytoken", { propertyId: property.id })
      ...
      <div className="border-t border-gray-100 pt-3">
        ...
        {level === 1 && !isFraud && (
          <div>
            ...
            {isOwner && (
              <Link
                href={`/verify/my-property/${property.id}`}
        ...
        {level >= 2 && level < 5 && !isFraud && (
          <div>
            ...
            <Link href={`/verify/${token?.tokenId || ""}`} className="text-xs font-bold text-[#FF6B00] hover:underline">
        ...
        {level === 5 && !isFraud && (
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              <Link href={`/verify/${token?.tokenId}`} className="flex items-center gap-1 bg-[#FF6B00] text-white font-bold text-xs px-3 py-2 rounded-lg hover:bg-[#e55f00] transition-colors">
