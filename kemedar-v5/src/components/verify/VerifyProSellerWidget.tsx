...
export default function VerifyProSellerWidget({ propertyId }) {
  ...
  useEffect(() => {
    ...
    apiClient.list("/api/v1/propertytoken", { propertyId })
  ...
  if (isFraud) {
    return (
      <div className="flex items-center gap-2 flex-wrap mt-1">
        ...
        <Link href="/contact" className="text-[10px] text-red-600 hover:underline font-bold">Contact Support</Link>
      </div>
    );
  }
    ...
    return (
      <div className="mt-1 flex flex-col gap-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Link href={`/verify/${token?.tokenId}`} className="text-[10px] text-[#FF6B00] hover:underline font-bold">View Certificate</Link>
        </div>
        {expiringSoon && (
          <div className="flex items-center gap-2 flex-wrap">
  ...
  if (level >= 2) {
    return (
      <div className="mt-1 flex items-center gap-2 flex-wrap">
        ...
        <Link href={`/verify/my-property/${propertyId}`} className="text-[10px] text-[#FF6B00] hover:underline font-bold">Continue Verifying →</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mt-1 flex items-center gap-2 flex-wrap">
      ...
      <Link href={`/verify/my-property/${propertyId}`} className="text-[10px] text-[#FF6B00] hover:underline font-bold">Start Verifying →</Link>
    </div>
  );
}