...
export default function AdminConciergeWidget() {
  ...
  useEffect(() => {
    Promise.all([
      base44.entities.ConciergeJourney.filter({ status: "Active" }),
      base44.entities.ConciergeTask.list("-updated_date", 200),
    ]).then(([journeys, tasks]) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/admin/kemedar/concierge" className="text-xs text-orange-500 font-bold hover:underline">
          View Full Dashboard →
        </Link>
      </div>
