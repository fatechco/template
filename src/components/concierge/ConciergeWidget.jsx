...
export default function ConciergeWidget({ userId }) {
  ...
  const loadData = async () => {
    try {
      const journeys = await base44.entities.ConciergeJourney.filter(
      ...
      const [props, tasks] = await Promise.all([
        j.propertyId ? base44.entities.Property.filter({ id: j.propertyId }) : Promise.resolve([]),
        base44.entities.ConciergeTask.filter({ journeyId: j.id }, "sortOrder", 50),
      ]);
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden" style={{ borderLeft: "4px solid #FF6B00" }}>
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to={`/dashboard/concierge/${journey.id}`} className="text-sm text-orange-500 font-bold hover:text-orange-600">
            View all →
          </Link>
        </div>
        ...
        {nextTask && (
          <div className="bg-orange-50 rounded-xl p-3 flex items-center gap-3">
            ...
            <Link
              to={`/dashboard/concierge/${journey.id}`}
