    ...
    try {
      ...
      if (form.nearby_distances && property?.id) {
        ...
        for (const [fieldId, d] of distEntries) {
          await base44.entities.PropertyDistance.create({
      ...
      if (auctionData?.enabled && property?.id) {
        try {
          const user = await base44.auth.me();
          const auctionRes = await base44.functions.invoke("createAuction", {
            propertyId: property.id,
      ...
      if (form.swapData?.enabled && property?.id) {
        try {
          const user = await base44.auth.me();
          const intent = await base44.entities.SwapIntent.create({
            userId: user.id,
          ...
          await base44.functions.invoke("publishSwapIntent", { swapIntentId: intent.id });
      ...
      if (form.fracEnabled && form.fracOfferingType && property?.id) {
        await base44.functions.invoke("submitFracOffering", {
          propertyId: property.id,
