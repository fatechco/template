...
import { Menu, X, User, FileText, DollarSign, HelpCircle, CreditCard, ClipboardList, BarChart3, Users, Calendar, MessageCircle, Bell, Settings, MapPin, HardHat, Plus, Star, Bookmark, ShoppingCart, LogOut, Search, Tag, Heart, GitCompare, ScanSearch, KanbanSquare, ChevronDown, ChevronRight } from "lucide-react";

const MENU = [
  { label: "Dashboard", icon: BarChart3, to: "/m/cp/user" },
  {
    label: "🏙 Kemedar", icon: MapPin, children: [
      { heading: "PROPERTIES" },
      { label: "Find Property", icon: Search, to: "/m/find/filters" },
      { label: "Browse Properties", icon: Search, to: "/search-properties" },
      { label: "My Properties", icon: FileText, to: "/m/cp/user/my-properties" },
      { label: "Add New Property", icon: Plus, to: "/create/property" },
      { label: "My Favorites", icon: Star, to: "/m/cp/user/favorites" },
      { label: "Compare Properties", icon: Settings, to: "/m/cp/user/compare" },
      { heading: "REQUESTS" },
      { label: "Add Buy Request", icon: Plus, to: "/create/buy-request" },
      { label: "My Buy Requests", icon: ClipboardList, to: "/m/cp/user/my-buy-requests" },
      { label: "Search Buy Requests", icon: Search, to: "/m/dashboard/search-requests" },
      { heading: "PROJECTS" },
      { label: "Find Project", icon: Search, to: "/search-projects" },
      { heading: "ORGANIZERS" },
      { label: "Buyer Organizer", icon: ClipboardList, to: "/m/cp/user/buyer-organizer" },
      { label: "Seller Organizer", icon: ClipboardList, to: "/m/cp/user/seller-organizer" },
    ]
  },
  {
    label: "🔧 Kemework", icon: HardHat, children: [
      ...
      { label: "My Tasks", icon: FileText, to: "/m/cp/user/kemework/my-tasks" },
      { label: "My Task Orders", icon: ClipboardList, to: "/m/cp/user/kemework/orders" },
      { heading: "FIND & BROWSE" },
      ...
      { heading: "SAVED" },
      { label: "Bookmarked Pros & Services", icon: Bookmark, to: "/m/cp/user/kemework/bookmarks" },
    ]
  },
  {
    label: "🛒 Kemetro", icon: ShoppingCart, children: [
      ...
      { heading: "MY ORDERS" },
      { label: "All Orders", icon: ClipboardList, to: "/m/cp/user/kemetro-orders" },
      { heading: "REQUESTS" },
      { label: "My RFQs", icon: FileText, to: "/m/cp/user/kemetro-rfqs" },
      { label: "Post New RFQ", icon: Plus, to: "/m/add/rfq" },
    ]
  },
  { label: "💎 Premium Services", icon: CreditCard, to: "/m/cp/user/subscription" },
  {
    label: "🤖 AI & Smart Tools", icon: Star, children: [
      { heading: "PROPERTY TOOLS" },
      { label: "AI Negotiations", icon: MessageCircle, to: "/m/cp/user/negotiations" },
      { label: "Escrow & Payments", icon: CreditCard, to: "/m/cp/user/escrow" },
      { label: "Property Valuations", icon: Search, to: "/m/cp/user/valuations" },
      { label: "Advisor Report", icon: FileText, to: "/m/cp/user/advisor-report" },
      { heading: "MY PROFILE" },
      { label: "Kemedar Score", icon: Star, to: "/m/cp/user/score" },
      { label: "My DNA Profile", icon: User, to: "/m/cp/user/my-dna" },
      { label: "KYC Verification", icon: KanbanSquare, to: "/m/cp/user/kyc" },
    ]
  },
  {
    label: "💰 Money & Orders", icon: DollarSign, children: [
      { heading: "KEMEDAR" },
      { label: "Kemedar Orders", icon: ClipboardList, to: "/m/cp/user/kemedar-orders" },
      { heading: "KEMETRO" },
      { label: "Kemetro Orders", icon: ShoppingCart, to: "/m/cp/user/kemetro-orders" },
      { label: "My RFQs", icon: FileText, to: "/m/cp/user/kemetro-rfqs" },
      { heading: "KEMEWORK" },
      { label: "Kemework Orders", icon: ClipboardList, to: "/m/cp/user/kemework/orders" },
      { heading: "PAYMENTS" },
      { label: "Wallet", icon: CreditCard, to: "/m/cp/user/wallet" },
      { label: "Invoices", icon: FileText, to: "/m/cp/user/invoices" },
      { label: "Payment Methods", icon: CreditCard, to: "/m/cp/user/payment-methods" },
    ]
  },
  {
    label: "🗂 Tools & Communications", icon: MessageCircle, children: [
      { heading: "COMMUNICATIONS" },
      { label: "Messages", icon: MessageCircle, to: "/m/cp/user/messages" },
      { label: "Notifications", icon: Bell, to: "/m/cp/user/notifications" },
      { heading: "ACCOUNT" },
      { label: "My Profile", icon: User, to: "/m/cp/user/profile" },
      { label: "Subscription & Billing", icon: CreditCard, to: "/m/cp/user/subscription" },
      { label: "Settings", icon: Settings, to: "/m/cp/user/settings" },
    ]
  },
  {
    label: "❓ Help", icon: HelpCircle, children: [
      { label: "Support Tickets", icon: Bell, to: "/m/cp/user/tickets" },
      { label: "Help Center & FAQ", icon: FileText, to: "/m/cp/user/knowledge" },
      { label: "Contact Us", icon: MessageCircle, to: "/m/cp/user/contact-kemedar" },
    ]
  },
];
...
const BOTTOM_TABS = [
  { id: 'settings', label: 'Settings', icon: Settings, path: '/m/cp/user/settings' },
...
export default function MobileCommonUserShell() {
  ...
  const isActive = (to) => to && (pathname === to || (to !== "/m/cp/user" && pathname.startsWith(to)));

  // Hide topbar for knowledge article detail pages (they have their own header)
  const hideTopBar = pathname.includes("/knowledge/") && pathname !== "/m/cp/user/knowledge";
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Bar - Hidden for knowledge article pages */}
        {!hideTopBar && (
          <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-30">
            ...
            <h2 className="font-bold text-gray-900 text-sm flex-1 text-center">Common User</h2>
            <div className="w-8" />
          </div>
        )}
