...
import {
  ...
  Plus, MapPin, HardHat, MessageCircle, Bell, Settings, Heart, GitCompare
} from "lucide-react";

const MENU = [
  { label: "Dashboard", icon: BarChart3, to: "/m/cp/pro" },
  {
    label: "My Services", icon: Star, children: [
      { label: "All Services", icon: FileText, to: "/m/cp/pro/services" },
      { label: "Add Service", icon: Plus, to: "/kemework/add-service" },
    ]
  },
  {
    label: "My Work", icon: Briefcase, children: [
      { label: "Orders", icon: ClipboardList, to: "/m/cp/pro/orders" },
      { label: "My Bids", icon: FileText, to: "/m/cp/pro/bids" },
      { label: "Search Tasks", icon: Search, to: "/m/cp/pro/search-tasks" },
    ]
  },
  {
    label: "Portfolio & Profile", icon: UserSquare, children: [
      { label: "My Profile", icon: User, to: "/m/cp/pro/profile" },
      { label: "Portfolio", icon: FileText, to: "/m/cp/pro/portfolio" },
      { label: "My Customers", icon: Users, to: "/m/cp/pro/customers" },
      { label: "Accreditation", icon: Star, to: "/m/cp/pro/accreditation" },
      { label: "Be Accredited", icon: Star, to: "/m/kemework/be-accredited" },
    ]
  },
  {
    label: "💰 Earnings", icon: DollarSign, children: [
      { label: "Earnings", icon: DollarSign, to: "/m/cp/pro/earnings" },
      { label: "Invoices", icon: FileText, to: "/m/cp/pro/invoices" },
    ]
  },
  {
    label: "🤖 AI & Smart Tools", icon: Star, children: [
      { heading: "BUILD & MATERIALS" },
      { label: "Kemetro Build™ — BOQ", icon: Briefcase, to: "/m/cp/pro/build" },
      { label: "Finish™ — Projects", icon: ClipboardList, to: "/m/cp/pro/finish/new" },
      { heading: "MY PROFILE" },
      { label: "Kemedar Score", icon: Star, to: "/m/cp/pro/score" },
    ]
  },
  {
    label: "🗂 Tools", icon: MessageCircle, children: [
      { heading: "COMMUNICATIONS" },
      { label: "Messages", icon: MessageCircle, to: "/m/cp/pro/messages" },
      { label: "Notifications", icon: Bell, to: "/m/cp/pro/notifications" },
      { heading: "ACCOUNT" },
      { label: "Subscription", icon: CreditCard, to: "/m/cp/pro/subscription" },
      { label: "Settings", icon: Settings, to: "/m/cp/pro/settings" },
    ]
  },
  {
    label: "❓ Help", icon: HelpCircle, children: [
      { label: "Support Tickets", icon: Bell, to: "/m/cp/pro/tickets" },
      { label: "Help Center", icon: FileText, to: "/m/cp/pro/knowledge" },
      { label: "Contact Us", icon: MessageCircle, to: "/m/cp/pro/contact-kemedar" },
    ]
  },
];
...
const BOTTOM_TABS = [
  { id: "settings", label: "Settings", icon: Settings, path: "/m/cp/pro/settings" },
  ...
  { id: "messages", label: "Messages", icon: MessageCircle, path: "/m/cp/pro/messages" },
  { id: "account",  label: "Account",  icon: User,     path: "/m/cp/pro/profile" },
];

export default function MobileProfessionalShell() {
  ...
  const isActive = (to) => to && (pathname === to || (to !== "/m/cp/pro" && pathname.startsWith(to)));
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-30">
          ...
          <h2 className="font-bold text-gray-900 text-sm flex-1 text-center">Professional</h2>
          <div className="w-8" />
        </div>
