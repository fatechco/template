...
export default function FinishProjectBadge({ propertyId, compact = false }) {
  ...
  useEffect(() => {
    ...
    base44.entities.FinishProject.filter({ propertyId })
  ...
  if (project) {
    ...
    if (compact) {
      return (
        <Link to={`/kemedar/finish/${project.id}`}
  ...
  return (
    <Link to="/kemedar/finish/new" className="block rounded-2xl border-2 border-dashed border-orange-300 p-4 hover:border-orange-500 hover:bg-orange-50 transition-all group">
