...
function LiveNavBadge() {
  ...
  useEffect(() => {
    import('@/api/base44Client').then(({ base44 }) => {
      base44.entities.LiveEvent.filter({ streamStatus: 'live', isApproved: true })
        .then(data => setHasLive(data.length > 0)).catch(() => {});
    });
  }, []);
  return (
    <Link to="/kemedar/live" className="flex items-center gap-1.5 text-sm font-semibold text-red-600 hover:bg-red-50 rounded-lg px-3 py-2 transition-colors whitespace-nowrap border border-red-200">
      ...
      <div className="max-w-[1400px] mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <div className="relative hidden lg:block group">
          ...
          <div className="absolute top-full left-0 mt-1 w-48 bg-white rounded-xl border border-gray-100 shadow-xl z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150">
            <Link to="/kemefrac" className="flex items-center gap-2 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50 rounded-t-xl">
              🏗️ All Offerings
            </Link>
            <Link to="/kemefrac/portfolio" className="flex items-center gap-2 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50">
              💼 My Portfolio
            </Link>
            <Link to="/kemefrac/kyc" className="flex items-center gap-2 px-4 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50 rounded-b-xl">
              🪪 KYC Verification
            </Link>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {isModuleActive('kemework') && (
            <Link to="/kemework/post-task" className="flex items-center gap-1.5 text-sm font-medium border-2 border-emerald-600 text-emerald-700 hover:bg-emerald-50 rounded-lg px-4 py-2 transition-colors whitespace-nowrap">
              <Plus size={15} />
              Post Task
            </Link>
          )}
          {isModuleActive('kemetro') && (
            <Link to="/kemetro/seller/products/add" className="flex items-center gap-1.5 text-sm font-medium border-2 border-blue-600 text-blue-700 hover:bg-blue-50 rounded-lg px-4 py-2 transition-colors whitespace-nowrap">
              <Plus size={15} />
              Sell Product
            </Link>
          )}
