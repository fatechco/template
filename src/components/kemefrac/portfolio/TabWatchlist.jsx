...
export default function TabWatchlist({ watchlist, setWatchlist }) {
  const handleRemove = async (id) => {
    ...
    await base44.entities.FracWatchlist.delete(id).catch(() => {});
  };

  const handleToggle = async (id, key, val) => {
    setWatchlist(w => w.map(item => item.id === id ? { ...item, [key]: val } : item));
    await base44.entities.FracWatchlist.update(id, { [key]: val }).catch(() => {});
  };
  ...
  if (!watchlist || watchlist.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center">
        ...
        <Link to="/kemefrac" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-black text-sm mt-5"
