...
export default function FlashDealCard({ deal }) {
  ...
  return (
    <Link to={`/kemetro/flash/deal/${deal.id}`} className="block group" onClick={e => e.stopPropagation()}>
      <div className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 group-hover:-translate-y-1">
        ...
        <div className="px-3 pb-3 space-y-2" onClick={e => e.preventDefault()}>
          ...
          <Link to={`/kemetro/flash/deal/${deal.id}`} className="block w-full bg-red-600 hover:bg-red-500 text-white font-black py-2.5 rounded-xl text-sm text-center transition-colors">
            ⚡ Order Now
          </Link>
        </div>
