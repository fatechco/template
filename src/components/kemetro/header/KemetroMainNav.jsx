...
function KemetroLogo() {
  return (
    <Link to="/kemetro" className="select-none flex-shrink-0">
      <img
      ...
      {open && (
        <div className="absolute right-0 top-full mt-0 bg-white rounded-xl shadow-2xl border border-gray-100 z-[200] w-48 py-2">
          {[
            ...
            <Link key={item.label} to={item.to} className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#0077B6] transition-colors">
              {item.label}
            </Link>
          ))}
        </div>
...
export default function KemetroMainNav({ cartCount = 0, wishlistCount = 0 }) {
  ...
  const handleSearch = (e) => {
    ...
    navigate(`/kemetro/search?${params.toString()}`);
  };
    ...
    <div className="w-full bg-white shadow-md border-b border-gray-100">
      <div className="max-w-[1400px] mx-auto px-4 h-16 flex items-center gap-4">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {/* Wishlist */}
          <Link to="/kemetro/wishlist" className="relative flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors text-gray-700">
            <Heart size={20} />
            {wishlistCount > 0 && (
          ...
          {/* Cart */}
          <Link to="/kemetro/cart" className="relative flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-orange-50 transition-colors text-[#FF6B00] font-semibold">
