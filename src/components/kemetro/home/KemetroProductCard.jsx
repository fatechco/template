...
export default function KemetroProductCard({ product, badge = null, badgeType = "featured" }) {
  ...
  return (
    <div className="flex-shrink-0 w-56 bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all group">
      {/* Image container */}
      <Link to={`/kemetro/product/${product.slug || product.id}`} className="relative aspect-square overflow-hidden bg-gray-100 block">
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="p-4 space-y-2">
        <Link to={`/kemetro/store/${product.storeId}`} className="text-xs text-blue-600 font-semibold hover:underline">
          {product.storeName}
        </Link>
        <Link to={`/kemetro/product/${product.slug || product.id}`} className="block font-bold text-gray-900 text-sm line-clamp-2 hover:text-[#FF6B00] transition-colors">
          {product.name}
        </Link>
