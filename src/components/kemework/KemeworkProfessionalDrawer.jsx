...
function NavItem({ item, onClose }) {
      ...
      <div>
        ...
        {open && (
          <div className="ml-9 mt-1 flex flex-col gap-0.5">
            {item.children.map(c => (
              <Link key={c.path} to={c.path} onClick={onClose} className="text-sm text-gray-600 px-3 py-2 rounded-lg hover:bg-gray-100 hover:text-gray-900 transition-colors">{c.label}</Link>
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <Link to={item.path} onClick={onClose} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-colors ${isActive ? "bg-teal-50 text-teal-700" : "hover:bg-gray-100 text-gray-800"}`}>
      <span className="text-lg w-6 flex-shrink-0">{item.icon}</span>
    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-72 bg-white z-50 shadow-2xl flex flex-col transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="border-t border-gray-100 p-4">
          <Link to="/kemework" onClick={onClose} className="block text-center text-xs font-bold text-teal-600 hover:text-teal-700">Go to Kemework Marketplace →</Link>
        </div>
      </div>
    </>
  );
