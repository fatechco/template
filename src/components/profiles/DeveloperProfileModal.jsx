import { X, MapPin, MessageCircle, Building2, Home, Layers, Phone } from "lucide-react";
...
export default function DeveloperProfileModal({ developer, index, onClose }) {
  if (!developer) return null;

  const logo = developer.logo || LOGOS[index % LOGOS.length];
  const projects = developer.projects_count || 8;
  const props = developer.properties_count || 120;
  const name = developer.name || developer.full_name || "Developer Name";
        ...
        <div className="px-6 py-5">
          ...
          <div className="flex gap-4 mb-5">
            <div className="flex-1 bg-blue-50 rounded-xl p-3 text-center">
              <Layers size={18} className="text-blue-600 mx-auto mb-1" />
              <p className="text-lg font-black text-gray-900">{projects}</p>
            ...
            <div className="flex-1 bg-gray-50 rounded-xl p-3 text-center">
              <Building2 size={18} className="text-gray-600 mx-auto mb-1" />
          ...
          {developer.email && (
            <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-xl px-4 py-2.5 mb-2">
              <Building2 size={14} className="text-[#FF6B00]" /> {developer.email}
            </div>
          )}
          {developer.phone && (
            <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-xl px-4 py-2.5 mb-2">
              <Phone size={14} className="text-[#FF6B00]" /> {developer.phone}
            </div>
          )}
