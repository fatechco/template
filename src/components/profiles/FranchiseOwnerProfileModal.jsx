import { X, MapPin, Phone, MessageCircle, Award, Users, BarChart2 } from "lucide-react";

const AVATARS = [
  "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=300&q=80",
  "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=300&q=80",
  "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=300&q=80",
  "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=300&q=80",
  "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=300&q=80",
  "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=300&q=80",
];

export default function FranchiseOwnerProfileModal({ owner, index, onClose }) {
  if (!owner) return null;

  const avatar = owner.avatar || AVATARS[index % AVATARS.length];
  const name = owner.full_name || owner.name || "Franchise Owner";

  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
        {/* Header band */}
        <div className="h-28 bg-gradient-to-r from-[#1a1a2e] to-[#2d2d4e] relative">
          <button onClick={onClose} className="absolute top-3 right-3 w-8 h-8 bg-white/20 hover:bg-white/40 text-white rounded-full flex items-center justify-center transition-colors">
            <X size={16} />
          </button>
          <span className="absolute top-3 left-4 bg-[#FF6B00] text-white text-[10px] font-bold px-2.5 py-1 rounded-full">
            FRANCHISE OWNER
          </span>
        </div>

        {/* Avatar */}
        <div className="flex flex-col items-center -mt-12 px-6 pb-6">
          <img src={avatar} alt={name} className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-lg mb-3" />
          <h2 className="text-xl font-black text-gray-900 text-center">{name}</h2>

          {/* Location */}
          {(owner.province_name || owner.city_name) && (
            <p className="text-sm text-gray-500 flex items-center gap-1.5 mt-1.5">
              <MapPin size={13} className="text-[#FF6B00]" />
              {[owner.province_name, owner.city_name].filter(Boolean).join(" — ")}
            </p>
          )}
          {owner.area_covered && (
            <p className="text-xs text-gray-400 mt-1 text-center">{owner.area_covered}</p>
          )}

          {/* Stats */}
          <div className="flex gap-4 mt-5 w-full">
            <div className="flex-1 bg-[#1a1a2e]/5 rounded-xl p-3 text-center">
              <Award size={18} className="text-[#FF6B00] mx-auto mb-1" />
              <p className="text-lg font-black text-gray-900">VIP</p>
              <p className="text-xs text-gray-500">Member</p>
            </div>
            <div className="flex-1 bg-[#1a1a2e]/5 rounded-xl p-3 text-center">
              <Users size={18} className="text-[#FF6B00] mx-auto mb-1" />
              <p className="text-lg font-black text-gray-900">{owner.users_count || "—"}</p>
              <p className="text-xs text-gray-500">Area Users</p>
            </div>
            <div className="flex-1 bg-[#1a1a2e]/5 rounded-xl p-3 text-center">
              <BarChart2 size={18} className="text-[#FF6B00] mx-auto mb-1" />
              <p className="text-lg font-black text-gray-900">Active</p>
              <p className="text-xs text-gray-500">Status</p>
            </div>
          </div>

          {owner.email && (
            <div className="flex items-center gap-2 mt-4 text-sm text-gray-600 w-full bg-gray-50 rounded-xl px-4 py-2.5">
              <Award size={14} className="text-[#FF6B00]" /> {owner.email}
            </div>
          )}
          {owner.phone && (
            <div className="flex items-center gap-2 mt-2 text-sm text-gray-600 w-full bg-gray-50 rounded-xl px-4 py-2.5">
              <Phone size={14} className="text-[#FF6B00]" /> {owner.phone}
            </div>
          )}

          <div className="flex gap-3 w-full mt-5">
            <button className="flex-1 flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-xl transition-colors text-sm">
              <MessageCircle size={15} /> Chat
            </button>
            <button className="flex-1 flex items-center justify-center gap-2 bg-[#1a1a2e] hover:bg-[#0d0d1a] text-white font-bold py-3 rounded-xl transition-colors text-sm">
              <Phone size={15} /> Call
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}