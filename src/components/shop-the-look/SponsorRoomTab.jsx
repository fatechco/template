...
function SponsorshipModal({ hotspot, property, imageUrl, onClose, sellerId, sellerProducts, onLaunch }) {
  ...
  const handleLaunch = async () => {
    ...
    setLaunching(true);
    try {
      await base44.entities.ImageHotspot.update(hotspot.id, {
        isSponsored: true,
...
function ShoppablePropertyCard({ property, sellerId, sellerProducts, onLaunch }) {
  ...
  const handleExpand = async () => {
    if (!expanded && analyzedImages.length === 0) {
      ...
      try {
        const imgs = await base44.entities.AnalyzedPropertyImage.filter(
          { propertyId: property.id, isAnalyzed: true, isShoppable: true }, "-created_date", 10
        );
        setAnalyzedImages(imgs || []);
        const map = {};
        await Promise.all((imgs || []).map(async img => {
          const hotspots = await base44.entities.ImageHotspot.filter({ imageId: img.id, isActive: true }, "sortOrder", 20);
          map[img.id] = hotspots || [];
        }));
...
export default function SponsorRoomTab({ sellerId }) {
  ...
  useEffect(() => {
    ...
    if (sellerId) {
      base44.entities.KemetroProduct.filter({ store_owner_id: sellerId, is_active: true }, "-created_date", 50)
  ...
  const loadShoppableProperties = async () => {
    ...
    try {
      ...
      const images = await base44.entities.AnalyzedPropertyImage.filter(
      ...
      const propList = await Promise.all(
        propIds.map(id => base44.entities.Property.filter({ id }).then(res => res?.[0]).catch(() => null))
      );
  ...
  const loadAnalytics = async () => {
    if (!sellerId) return;
    try {
      const logs = await base44.entities.HotspotSponsorshipLog.filter({ sellerId }, "-recordedAt", 500);
      const impressions = (logs || []).filter(l => l.eventType === "impression").length;
      const clicks = (logs || []).filter(l => l.eventType === "click").length;
      const carts = (logs || []).filter(l => l.eventType === "add_to_cart").length;
      const spent = (logs || []).reduce((s, l) => s + (l.costEGP || 0), 0);
  ...
  const handleToggleStatus = async (hotspot) => {
    ...
    await base44.entities.ImageHotspot.update(hotspot.id, { sponsorshipStatus: newStatus });
