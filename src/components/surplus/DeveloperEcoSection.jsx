    ...
    const load = async () => {
      try {
        const [scoreData, itemData] = await Promise.all([
          base44.entities.DeveloperEcoScore.filter({ developerUserId }, "-created_date", 1),
          base44.entities.SurplusItem.filter({ sellerId: developerUserId, status: "active" }, "-created_date", 10),
        ]);
