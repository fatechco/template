    ...
    const load = async () => {
      try {
        const [intents, matches, negs] = await Promise.all([
          ...
          base44.entities.SwapMatch.list("-created_date", 50),
          base44.entities.SwapMatch.filter({ status: "negotiating" }, "-created_date", 20),
        ]);
        setStats({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/admin/kemedar/swaps" className="text-xs text-[#7C3AED] font-bold hover:underline">View Full Dashboard →</Link>
      </div>
