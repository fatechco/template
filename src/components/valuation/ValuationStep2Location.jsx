...
export default function ValuationStep2Location({ data, onChange, onNext, onBack }) {
  ...
  useEffect(() => {
    base44.entities.Country.list('name', 50).then(setCountries).catch(() => {});
  }, []);

  useEffect(() => {
    if (data.countryId) {
      base44.entities.Province.filter({ country_id: data.countryId }, 'name', 50).then(setProvinces).catch(() => {});
  ...
  useEffect(() => {
    if (data.provinceId) {
      base44.entities.City.filter({ province_id: data.provinceId }, 'name', 100).then(setCities).catch(() => {});
    }
  }, [data.provinceId]);

  useEffect(() => {
    if (data.cityId) {
      base44.entities.District.filter({ city_id: data.cityId }, 'name', 100).then(setDistricts).catch(() => {});
    }
  }, [data.cityId]);

  useEffect(() => {
    if (data.districtId) {
      base44.entities.Area.filter({ district_id: data.districtId }, 'name', 100).then(setAreas).catch(() => {});
    }
  }, [data.districtId]);
