...
export default function VisionPhotoStep({ propertyId, propertyData, photos, onChange }) {
  ...
  useEffect(() => {
    ...
    base44.entities.PropertyPhotoAnalysis.filter({ propertyId }).then(setAnalyses).catch(() => {});
    base44.entities.PropertyVisionReport.filter({ propertyId }).then(r => r[0] && setReport(r[0])).catch(() => {});
  }, [propertyId]);
  ...
  const handleFilesSelected = async (files) => {
    ...
    for (const file of files) {
      ...
      // Trigger analysis
      if (propertyId) {
        base44.functions.invoke('analyzePropertyPhoto', {
          photoUrl: file_url,
          propertyId,
  ...
  const handleLabelSave = async (data) => {
    ...
    const updated = await base44.entities.PropertyPhotoAnalysis.update(labelModal.analysis.id, data);
    setAnalyses(prev => prev.map(a => a.id === updated.id ? updated : a));
    setLabelModal(null);
  };

  const handleRequestStaging = async (analysisId, style) => {
    await base44.functions.invoke('generateVirtualStaging', { photoAnalysisId: analysisId, style, propertyId });
    // Reload analyses
    if (propertyId) {
      const fresh = await base44.entities.PropertyPhotoAnalysis.filter({ propertyId });
      setAnalyses(fresh);
    }
  };
  ...
  const handleGenerateReport = async () => {
    if (!propertyId) return;
    const res = await base44.functions.invoke('generateVisionReport', { propertyId });
