...
const REVENUE_DATA = [
  { day: "Mar 1", amount: 0 }, { day: "Mar 5", amount: 420 }, { day: "Mar 8", amount: 280 },
  { day: "Mar 12", amount: 850 }, { day: "Mar 15", amount: 620 }, { day: "Mar 18", amount: 1090 },
  { day: "Mar 20", amount: 960 },
];
...
export default function CompanyDashboardDesktop() {
  ...
  return (
    <div className="space-y-6">
      {/* Greeting Card */}
      <div className="rounded-2xl p-6" style={{ background: "linear-gradient(135deg, #ec4899 0%, #db2777 100%)" }}>
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-3xl font-black text-white mb-2">Welcome back, {firstName}! 🏢</p>
            <span className="inline-block text-xs font-black px-3 py-1 rounded-full bg-pink-500/20 text-pink-100">Finishing Company</span>
          </div>
          <span className="text-xs font-bold px-3 py-1 rounded-full bg-amber-500/20 text-amber-100">🏅 Accredited</span>
        </div>
        ...
        <div className="bg-white/10 rounded-xl p-4 flex items-center justify-between mt-4">
          <div>
            ...
            <p className="text-pink-100 text-sm mt-1">Clients can {available ? "contact" : "not contact"} you for new work</p>
          </div>
          <button onClick={() => setAvailable(a => !a)} className={`w-16 h-8 rounded-full transition-colors relative flex-shrink-0 ${available ? "bg-pink-400" : "bg-gray-600"}`}>
      ...
      <div className="grid grid-cols-6 gap-4">
        {[
          { icon: "📋", label: "Active Projects", value: 8, color: "text-pink-600" },
          { icon: "👥", label: "Team Members", value: 12, color: "text-blue-600" },
          { icon: "✅", label: "Completed", value: 156, color: "text-green-600" },
          { icon: "💰", label: "This Month", value: "$12,340", color: "text-pink-600" },
          { icon: "⭐", label: "Rating", value: "4.8", color: "text-amber-500" },
          { icon: "👁", label: "Profile Views", value: 342, color: "text-purple-600" },
        ].map(s => (
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2">
          <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/cp/company/earnings" className="text-sm font-bold text-pink-600">View Report →</Link>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={REVENUE_DATA}>
                  <Line type="monotone" dataKey="amount" stroke="#ec4899" strokeWidth={3} dot={false} />
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
            <p className="font-black text-gray-900 text-sm mb-3">Performance</p>
            <div className="space-y-2 text-sm">
              <p className="flex justify-between"><span className="text-gray-500">Projects Done</span><span className="font-bold text-gray-900">156</span></p>
              <p className="flex justify-between"><span className="text-gray-500">On-Time Delivery</span><span className="font-bold text-pink-600">99%</span></p>
              <p className="flex justify-between"><span className="text-gray-500">Client Satisfaction</span><span className="font-bold text-green-600">98%</span></p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <p className="font-black text-gray-900 text-lg">New Tasks in Your Category 🔥</p>
          <Link to="/kemework/tasks" className="text-sm font-bold text-pink-600">View All →</Link>
        </div>
          ...
          {MOCK_TASKS.map(task => (
            <div key={task.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-4">
                ...
                <p className="text-sm font-black mb-3 text-pink-600">${task.budgetMin}–${task.budgetMax}</p>
                <Link to={`/kemework/task/${task.id}`} className="block text-center text-sm font-bold py-2 rounded-lg text-white bg-pink-600 hover:bg-pink-700 transition-colors">Bid Now</Link>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/cp/company/orders" className="text-sm font-bold text-pink-600">View All →</Link>
          </div>
          <div className="space-y-3">
            {MOCK_ORDERS.map(o => (
              <div key={o.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-pink-100 flex items-center justify-center text-sm font-black text-pink-600 flex-shrink-0">{o.client[0]}</div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/cp/company/bids" className="text-sm font-bold text-pink-600">View All →</Link>
          </div>
          <div className="space-y-3">
            {MOCK_BIDS.map(b => (
