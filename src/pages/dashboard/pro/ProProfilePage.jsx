...
const INPUT = "w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-teal-400 transition-colors";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            {/* Cover */}
            <div className="h-24 bg-gradient-to-r from-teal-500 to-teal-600 relative">
            ...
            <div className="px-5 pb-5">
              <div className="relative -mt-8 mb-3 w-fit">
                <div className="w-16 h-16 rounded-full bg-teal-600 text-white font-black text-xl flex items-center justify-center border-4 border-white shadow">
                  {(form.firstName[0] || "P").toUpperCase()}
                </div>
                <button className="absolute bottom-0 right-0 w-6 h-6 bg-teal-500 text-white rounded-full flex items-center justify-center">
                  <Camera size={11} />
                </button>
              </div>
              ...
              <a href={`/u/${form.slug || "your-profile"}`} className="mt-3 flex items-center gap-1.5 text-xs text-teal-600 hover:underline">
        ...
        <div className="lg:col-span-2 space-y-5">
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {specializations.map(spec => (
                  <span key={spec} className="px-3 py-1.5 bg-teal-100 text-teal-700 rounded-full text-xs font-bold flex items-center gap-2">
                    {spec}
                    <button onClick={() => setSpecializations(specializations.filter(s => s !== spec))} className="hover:text-teal-900">
                      <X size={12} />
                    </button>
                  </span>
                ))}
            ...
            <div className="space-y-3">
              {[
                ...
                <label key={key} className="flex items-center gap-3 cursor-pointer group">
                  <input type="checkbox" checked={form[key]} onChange={set(key)} className="w-4 h-4 accent-teal-500" />
          ...
          <button className="flex items-center gap-2 bg-teal-500 hover:bg-teal-600 text-white font-bold px-6 py-3 rounded-xl transition-colors">
