...
export default function MobileSettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6]">
      {/* Top App Bar */}
      <div className="sticky top-0 z-40 bg-white border-b border-[#E5E7EB] px-5 flex items-center justify-between" style={{ minHeight: 56 }}>
        <h1 className="font-black text-[#1F2937] text-lg">Settings</h1>
      ...
      <div className="px-4 py-5 space-y-5 pb-32">
        ...
        <SettingsSection label="ACCOUNT">
          <SettingsNavItem emoji="👤" label="Edit Profile" onPress={() => {}} />
          <div className="h-px bg-[#F3F4F6] mx-4" />
        ...
        {/* SECTION 6 — ABOUT & BENEFITS */}
        <SettingsSection label="ABOUT & BENEFITS">
          <SettingsNavItem emoji="🌟" label="User Benefits" value="Discover what Kemedar offers you" onPress={() => navigate('/m/benefits')} />
          <div className="h-px bg-[#F3F4F6] mx-4" />
          <SettingsNavItem emoji="ℹ️" label="About Kemedar" value="Our story and ecosystem" onPress={() => navigate('/m/about')} />
        </SettingsSection>
        ...
        <SettingsSection label="LEGAL">
          ...
          <div className="h-px bg-[#F3F4F6] mx-4" />
          <SettingsNavItem emoji="📞" label="Contact Support" onPress={() => {}} />
        </SettingsSection>
      ...
      {sheet === "language" && (
        <LanguageSheet
          selected={language}
          onSelect={(l) => { setLanguage(l); setSheet(null); }}
      ...
      {sheet === "currency" && (
        <CurrencySheet
          selected={currency}
          onSelect={(c) => { setCurrency(c); setSheet(null); }}
          onClose={() => setSheet(null)}
        />
      )}
      {sheet === "country" && (
        <CountrySheet
          selected={country}
          onSelect={(c) => { setCountry(c); setSheet(null); }}
          onClose={() => setSheet(null)}
        />
      )}
    </div>
