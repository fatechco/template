...
function StatusActions({ status, onNavigate, orderId }) {
  ...
  if (status === "in_progress") return (
    <>
      <button onClick={() => onNavigate(`/m/cp/agent/kemework/orders/${orderId}`)} className={`${btn} text-blue-600`}>👁 View Progress</button>
      <div className={div} />
  ...
  if (status === "completed") return (
    <>
      <button onClick={() => onNavigate(`/m/cp/agent/kemework/orders/${orderId}`)} className={`${btn} text-orange-600`}>⭐ Review</button>
      <div className={div} />
  ...
  if (status === "disputed") return (
    <>
      <button onClick={() => onNavigate(`/m/cp/agent/kemework/orders/${orderId}`)} className={`${btn} text-red-600`}>👁 View Dispute</button>
      <div className={div} />
...
export default function AgentKemeworkOrdersPage() {
