...
const MODULE_TABS = ["🏠 Kemedar", "🛒 Kemetro", "🔧 Kemework"];
...
export default function FranchiseCPMobileDashboard() {
          ...
          {MODULE_TABS.map((tab, i) => (
            <button key={i} onClick={() => setActiveModule(i)}
              className={`flex-1 py-2.5 text-xs font-bold border-b-2 transition-colors ${
                activeModule === i ? "text-orange-600 border-orange-600" : "text-gray-500 border-transparent"
              }`}
            >
              {tab}
            </button>
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
            {AREA_STATS.map((s, i) => (
              <div key={i} className={`flex-shrink-0 w-44 ${s.bg} rounded-2xl p-4 text-white`}>
                ...
                <button onClick={() => navigate('/m/cp/franchise/area-overview')} className="text-xs font-bold text-white/80 mt-2">View →</button>
              </div>
            ))}
          </div>
        </div>
