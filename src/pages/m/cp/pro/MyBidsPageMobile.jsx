...
import { X, ChevronDown, Star, DollarSign, Calendar } from "lucide-react";
...
const MOCK_BIDS = [
  ...
  { id: 4, task: "Garden Landscaping & Irrigation System", category: "Landscaping", amount: 3200, currency: "USD", days: 14, status: "Pending", date: "Mar 19", clientRating: null, image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd58?w=200&q=70", description: "Complete landscape design with irrigation system installation." },
...
export default function MyBidsPageMobile() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 max-w-[480px] mx-auto">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-4 sticky top-0 z-30">
        ...
        <p className="text-xs text-gray-500 mt-0.5">{MOCK_BIDS.length} bids submitted</p>
      </div>
        ...
        {filtered.map(bid => (
          <div key={bid.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <button
              ...
              className="w-full p-3 flex gap-3 hover:bg-gray-50 transition-colors text-left"
            >
              <div className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
                {bid.image ? <img src={bid.image} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-lg">🔧</div>}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  <span className="font-bold text-gray-700">${bid.amount}</span> · {bid.days}d
                </p>
              </div>

              <ChevronDown size={16} className={`text-gray-400 transition-transform flex-shrink-0 mt-1 ${expandedBid === bid.id ? "rotate-180" : ""}`} />
            </button>
            ...
            {expandedBid === bid.id && (
              <div className="border-t border-gray-100 px-4 py-4 space-y-3">
                {/* Description */}
                <div>
                  <p className="text-xs font-bold text-gray-700 mb-1.5">Description</p>
                ...
                <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 flex items-center gap-1"><DollarSign size={14} /> Bid Amount</span>
                    <span className="font-bold text-gray-900">${bid.amount}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600 flex items-center gap-1"><Calendar size={14} /> Duration</span>
                    <span className="font-bold text-gray-900">{bid.days} days</span>
                  </div>
                  {bid.clientRating && (
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600 flex items-center gap-1"><Star size={14} /> Client Rating</span>
                      <span className="font-bold text-gray-900">{bid.clientRating} ⭐</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Submitted</span>
                    <span className="font-bold text-gray-900">{bid.date}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2 pt-2">
                  {bid.status === "Pending" && (
                    <>
                      ...
                      <button className="w-full py-2.5 rounded-lg text-xs font-bold border border-red-200 text-red-600 hover:bg-red-50">
                        Withdraw Bid
                      </button>
                    </>
                  )}
                  {bid.status === "Accepted" && (
                    <Link
                      to="/m/cp/pro/orders"
                      className="w-full py-2.5 rounded-lg text-xs font-bold text-white text-center"
                      style={{ background: "#0D9488" }}
                    >
                      View Order →
                    </Link>
                  ...
                  {bid.status === "Rejected" && (
                    <Link
                      to={`/kemework/task/${bid.id}`}
                      className="w-full py-2.5 rounded-lg text-xs font-bold border border-gray-300 text-gray-600 text-center hover:bg-gray-50"
                    >
                      View Task
                    </Link>
                  )}
