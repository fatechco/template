...
const MOCK_PROMOTION = {
  ...
  endDate: "2026-04-01",
  maxUses: "500",
  description: "Special spring promotion offering 20% off on all products.",
};
...
export default function SellerPromotionsEditPage() {
  const navigate = useNavigate();
  const { id } = useParams();
  const [promoType, setPromoType] = useState(MOCK_PROMOTION.type);
  const [form, setForm] = useState({
    name: MOCK_PROMOTION.name,
    discount: MOCK_PROMOTION.discount,
    minOrder: MOCK_PROMOTION.minOrder,
    startDate: MOCK_PROMOTION.startDate,
    endDate: MOCK_PROMOTION.endDate,
    maxUses: MOCK_PROMOTION.maxUses,
  ...
  const handleSubmit = () => {
    ...
    navigate(`/m/dashboard/seller-promotions/${id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F0F7FF]">
      ...
      <div className="sticky top-0 z-30 bg-white flex items-center px-4 gap-3" style={{ height: 56, boxShadow: "0 1px 4px rgba(0,0,0,0.1)" }}>
        ...
        <span className="flex-1 font-black text-base text-gray-900 text-center">Edit Promotion</span>
        <div className="w-6" />
      </div>
