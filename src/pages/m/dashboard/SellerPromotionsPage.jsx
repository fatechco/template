...
import { ArrowLeft, Tag, Percent, Gift, Zap, Plus } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#F0F7FF]">
      ...
      <div className="sticky top-0 z-30 bg-white flex items-center px-4 gap-3" style={{ height: 56, boxShadow: "0 1px 4px rgba(0,0,0,0.1)" }}>
        ...
        <button onClick={() => navigate("/m/dashboard/seller-promotions/add")} className="p-1"><Plus size={22} className="text-[#0077B6]" /></button>
      </div>
              ...
              return (
                <div key={promo.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => navigate(`/m/dashboard/seller-promotions/${promo.id}`)} className="flex-1 bg-gray-100 text-gray-700 text-xs font-bold py-2.5 rounded-xl">View</button>
                    <button onClick={() => navigate(`/m/dashboard/seller-promotions/${promo.id}/edit`)} className="flex-1 bg-[#0077B6] text-white text-xs font-bold py-2.5 rounded-xl">Edit</button>
                  </div>
                </div>
              );
            })}
